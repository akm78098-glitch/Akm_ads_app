...
// Helper function to get user's orders (works for both advertisers and channel owners)
async function getUserOrders(userId, userType, db) {
    if (userType === 'advertiser') {
        // First get user's campaigns, then get orders for each campaign
        ...
        // Channel owner - get orders by channel
...
async function lockPayment(id) {
    try { 
        const res = await fetch(`${API_BASE}/api/orders/${id}/lock`, { method: 'POST' });
...
async function addFunds() {
    ...
    if (amount && amount >= 10 && amount <= 1000) {
        ...
        if (window.loadWallet) loadWallet();
    }
}
...
async function createCampaign() {
    ...
    alert('Campaign created! Auto-matching channels...'); 
    closeModal(); 
    loadCampaigns();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (path.includes('dashboard')) loadDashboard();
        ...
        loadCampaigns(); 
        document.getElementById('createCampaignBtn')?.addEventListener('click', showCreateModal); 
        document.getElementById('submitCampaign')?.addEventListener('click', createCampaign); 
